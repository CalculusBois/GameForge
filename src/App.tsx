...
function SkinPortrait({ id }: { id: SkinId }) {
    ...
    return (
        <svg viewBox="0 0 80 90" className="skin-art" aria-label={skin.name}>
            ...
            {["knight", "wanderer"].includes(id) && (
                <path
                    d="M24 30 L11 76 L46 67 L55 33"
                    fill={id === "knight" ? "#392a53" : "#6f5c9c"}
                />
            )}
            <path
                d="M24 35 H57 V62 H24Z M18 36 H25 V58 H18Z M57 36 H64 V58 H57Z"
                fill={skin.color}
            />
            <path
                d="M29 62 H38 V80 H25V74H29Z M45 62H54V74H58V80H45Z"
                fill="#96a9ad"
            />
            ...
            {id === "ranger" && (
                <path
                    d="M22 12H64V18H22Z M31 5H53V12H31Z M10 38H17V65H10Z"
                    fill="#9d7755"
                />
            )}
            {id === "wanderer" && (
                <path
                    d="M25 14L29 0L36 12L43 0L48 12L55 0L59 14Z"
                    fill="#d5bdff"
                />
            )}
            {id === "neon" && (
                <path
                    d="M21 3H24V24H21Z M60 3H63V24H60Z M12 40H18V63H12Z M39 43H42V58H39Z"
                    fill="#a8ffed"
                />
            )}
            {id === "knight" && (
                <path
                    d="M20 0L30 14H23Z M63 0L54 14H61Z M18 31H31V39H18Z M54 31H68V39H54Z"
                    fill="#c593b5"
                />
            )}
            {id === "automaton" && (
                <>
                    ...
                    <path
                        d="M29 17H37V23H29Z M49 17H57V23H49Z"
                        fill="#beffee"
                    />
...
const EMPTY: SandboxHud = {
    ...
    selected: 1,
    message: "Explore Lumen frontier",
    nearBase: true,
    fps: 0,
    chunks: 0,
    enemies: 0,
...
export default function App() {
    ...
    const [gameState, setGameState] = useState<"home" | "worlds" | "playing">(
        "home",
    );
    ...
    const createAndEnterWorld = (fromSettings: WorldSettings = settings) =>
        act(async () => {
            gameAudio.unlock();
            const seed =
                fromSettings.seed.trim() ||
                `WORLD-${Date.now().toString(36).toUpperCase()}`;
        ...
        const click = (event: MouseEvent) => {
            if (
                (event.target as HTMLElement | null)?.closest(
                    "button, summary, .item-slot, .import-label",
                )
            )
                gameAudio.ui();
        };
    ...
    if (!store)
        return (
            <main className="loading">
                ...
                <p>{error || "Opening local world storage."}</p>
                {error && (
                    <p>
                        Close other game tabs and reload. Incompatible saves are
    ...
    const runSearch = () => {
        ...
        const setCmd =
            /^\/set\s+(health|hp|mana|hunger|food)\s+(max|-?\d+)\s*$/i.exec(
                query,
            );
        if (setCmd) {
            ...
            const parseVal = (maxV: number) =>
                raw === "max" ? maxV : Number(raw);
            let toast: {
                text: string;
                kind: "health" | "mana" | "hunger" | "info";
            } | null = null;
            if (kind === "health" || kind === "hp") {
                ...
                if (
                    ...
                    value < 0 ||
                    (raw !== "max" && !Number.isSafeInteger(value))
                )
                    toast = { text: "Invalid health value.", kind: "info" };
                else {
                    const clamped = Math.min(
                        Math.max(0, Math.floor(value)),
                        Math.max(stats.maxHealth, 9999),
                    );
                    ...
                    toast = {
                        text:
                            raw === "max"
                                ? `Health → ${stats.maxHealth}/${stats.maxHealth}`
                                : `Health → ${clamped}/${stats.maxHealth}`,
                        kind: "health",
                    };
                }
            } else if (kind === "mana") {
                ...
                if (
                    ...
                    toast = { text: "Invalid mana value.", kind: "info" };
                else {
                    const clamped = Math.min(
                        Math.max(0, Math.floor(value)),
                        stats.maxMana,
                    ...
                    toast = {
                        text:
                            raw === "max"
                                ? `Mana → ${stats.maxMana}/${stats.maxMana}`
                                : `Mana → ${clamped}/${stats.maxMana}`,
                        kind: "mana",
                    };
                }
            } else {
                ...
                if (
                    ...
                    toast = { text: "Invalid hunger value.", kind: "info" };
                else {
                    const clamped = Math.min(
                        100,
                        Math.max(0, Math.floor(value)),
        ...
        const give = /^\/give\s+([a-z0-9_]+)(?:\s+([+-]?\d+))?\s*$/i.exec(
            query,
        );
        if (give) {
            ...
            const aliases: Record<string, ItemId> = {
                ...
                gold_ore_item: "gold_ore",
                goldbar: "gold_bar",
                cobalt: "cobalt_ore",
                cobaltbar: "cobalt_bar",
                timber: "wood",
                logs: "wood",
                log: "wood",
                healing_herb: "herb",
            ...
            if (!Number.isSafeInteger(amount) || amount === 0) {
                ...
                const id =
                    aliases[raw] ?? (raw in ITEMS ? (raw as ItemId) : null);
                if (!id) setNotice(`Unknown item "${raw}".`);
                    ...
                    void act(async () => {
                        await store.transact((_b, w) => {
                            if (
                                amount > 0
                                    ? !add(w.inventory, id, amount)
                                    : !remove(w.inventory, id, -amount)
                            )
                                throw new Error(
                                    amount > 0
                                        ? "Inventory full — clear space first."
                                        : `Not enough ${ITEMS[id].name}.`,
                                );
            ...
            <main className="worlds-select">
                <header className="worlds-select-header">
                    <div>
                        ...
                        <p>
                            Worlds keep their own progress. Shared cosmetics
                            remain available in every expedition.
                        </p>
                    </div>
                    ...
                    {worldsList.map((w) => (
                        <article className="world-card" key={w.id}>
                            <div>
                                ...
                                <p>
                                    ◈ {w.coins} coins ·{" "}
                                    {Object.keys(w.explored).length} chunks
                                    explored
                                </p>
                            </div>
                            <button
                        ...
                        <label>
                            ...
                            <button
                                ...
                                onClick={() =>
                                    setSettings({
                                        ...settings,
                                        seed: `WORLD-${Math.random().toString(36).slice(2, 8).toUpperCase()}`,
                                    })
                                }>
                                Random
                            </button>
                            ...
                            <select
                                ...
                                onChange={(e) =>
                                    setSettings({
                                        ...settings,
                                        difficulty: e.target
                                            .value as WorldSettings["difficulty"],
                                    })
                                }>
                    ...
                    {worldsList.length > 0 && (
                        <div className="world-delete-panel">
                            ...
                            <button
                                ...
                                onClick={() => {
                                    const selected =
                                        store.data.worlds[deleteTarget];
                                    if (
                                        selected &&
                                        confirm(
                                            `Delete world “${selected.name}”? This cannot be undone.`,
                                        )
                                    )
                                        void deleteWorld(deleteTarget);
                                }}>
                                Delete selected
                            </button>
                        </div>
        ...
        <main className="sandbox-app">
            <header className="sandbox-header">
                ...
                <div className="header-right">
                    ...
                    <button
                        type="button"
                        onClick={downloadCsv}
                        title="Export play data as CSV for SIGNAL analysis">
                        Export to SIGNAL
                    </button>
                    <button
                        onClick={() => open("forge")}
            ...
            <section className="world-shell" ref={frame}>
                ...
                <div
                    ...
                    <div
                        className={`vital${cmdToast?.kind === "health" ? " vital-flash" : ""}`}>
                        <span>
                            HEALTH{" "}
                            <b>
                                {hud.health}/{hud.maxHealth || 100}
                            </b>
                        </span>
                        <meter
                            min="0"
                            max={Math.max(hud.maxHealth || 100, hud.health)}
                            value={hud.health}
                        />
                    </div>
                    <div
                        className={`vital mana${cmdToast?.kind === "mana" ? " vital-flash" : ""}`}>
                        <span>
                            MANA{" "}
                            <b>
                                {hud.mana}/{hud.maxMana || 100}
                            </b>
                        </span>
                        <meter
                            min="0"
                    ...
                    <div
                        className={`vital${cmdToast?.kind === "hunger" ? " vital-flash" : ""}`}>
                        <span>
                            HUNGER <b>{Math.ceil(hud.hunger)}/100</b>
                        </span>
                    ...
                    {hud.activeEffects && hud.activeEffects.length > 0 && (
                        <div
                            ...
                            {hud.activeEffects.map((eff, i) => (
                                <span
                                    key={i}
                                    title={`${eff.name} (${Math.ceil(eff.remainingMs / 1000)}s)`}
                                    style={{
                                        background: "#1c2833",
                                    ...
                                    <span
                                        ...
                                        {eff.remainingMs > 60000
                                            ? "∞"
                                            : `${Math.ceil(eff.remainingMs / 1000)}s`}
                                    </span>
                                </span>
                            ))}
                        </div>
                    ...
                    <div className="location">
                        ...
                        <small>
                            {world.name} · {Math.floor(hud.y / TILE)}m depth
                        </small>
                    </div>
                    <div className="coins">
                        ◈ {world.coins}
                    ...
                    <button
                        ...
                        onPointerDown={(e) => e.preventDefault()}
                        onClick={() => {
                            if (hud.status === "paused" && !menu) resumeGame();
                            else {
                                engine.current?.pause();
                                gameAudio.stopBgm();
                                setMenu(null);
                        ...
                        {hud.status === "paused" && !menu ? "Play" : "Pause"}
                    </button>
                    <button
                        type="button"
                        className="hud-fullscreen-btn"
                ...
                <div className="viewport">
                    <div
                        ref={host}
                        className="game-host"
                        aria-label="Lumen frontier sandbox game"
                    />
                    {import.meta.env.DEV && debug && (
                        <div className="debug">
                            {hud.fps} FPS · {hud.chunks}/15 chunks ·{" "}
                            {hud.enemies}/12 enemies · {hud.bodies} terrain
                            bodies · {Math.floor(hud.x)},{Math.floor(hud.y)}
                        </div>
                    )}
                </div>
                {showSearch && (
                        ...
                        <section className="tutorial-card">
                            ...
                            <div className="tutorial-grid">
                                <article>
                                    ...
                                    <h3>Move</h3>
                                    <p>A/D or arrow keys walk. Space jumps.</p>
                                </article>
                                <article>
                                    <TutorialClip kind="mine" />
                                    <h3>Mine & place</h3>
                                    <p>
                                        J or click mines. F or right-click
                                        places blocks.
                                    </p>
                                </article>
                                <article>
                                    ...
                                    <h3>Fight</h3>
                                    <p>
                                        Weapons fire toward your pointer. Watch
                                        health, mana, hunger, and effects.
                                    </p>
                                </article>
                                <article>
                                    ...
                                    <h3>Gather</h3>
                                    <p>
                                        Hold E to chop trees (~1.2s) and forage
                                        plants. Tap E for chests and stations.
                                    </p>
                                </article>
                                <article>
                                    ...
                                    <h3>Menus</h3>
                                    <p>
                                        I inventory · C crafting · M map · 1–8
                                        selects gear.
                                    </p>
                                </article>
                                <article>
                                    ...
                                    <h3>Recall</h3>
                                    <p>
                                        Press H to recall to the outpost. Damage
                                        cancels recall.
                                    </p>
                                </article>
                                <article>
                            ...
                            <div>
                                <p className="eyebrow">
                                    {hud.status === "dead"
                                        ...
                                        : "LUMEN OUTPOST / EXPEDITION PAUSED"}
                                </p>
                                <h2>
                                    {hud.status === "dead"
                                ...
                                <p>
                                    {hud.status === "dead"
                                        ? "Respawn at the outpost. Your inventory, coins, and skins are retained."
                                        : "Explore living hills and buried ruins. Gather, craft, and make this planet your own."}
                                </p>
                                <button
                                    type="button"
                                    className="primary"
                                ...
                                <p className="start-tip">
                                    Hold E chops trees (~1.2s) · J mines · 1–8
                                    selects equipment
                                </p>
                            </div>
                        </div>
                    ...
                    <div className="menu-shade">
                        <section
                            ...
                            <div className="menu-content">
                                {menu === "storage" && hud.storageKey && (
                                    <>
                                        <p>
                                            Click a pack stack to deposit it;
                                            click a stored stack to withdraw it.
                                            Nonempty crates cannot be mined.
                                        </p>
                                        <h3>Pack</h3>
                                ...
                                {menu === "inventory" && (
                                    <div>
                                        <button
                                            onClick={() =>
                                                void transact((_b, w) => {
                                                    sortPack(w.inventory);
                                                    return "Sorted pack; hotbar preserved";
                                                })
                                            }>
                                            Sort pack
                                        </button>
                                        ...
                                        <p>
                                            Eat food from your hotbar with J.
                                            Cooked meal bonuses replace the
                                            previous meal.
                                        </p>
                                        {world.meal && (
                                            <p>
                                ...
                                {menu === "inventory" && (
                                    <div
                                        ...
                                        <div
                                            ...
                                            {(() => {
                                                const stats =
                                                    derivePlayerStats(world);
                                                const vitCores =
                                                    world.upgrades
                                                        ?.vitalityCores ?? 0;
                                                const manaCores =
                                                    world.upgrades?.manaCores ??
                                                    0;
                                                const hasVitCore =
                                                    count(
                                                        world.inventory,
                                        ...
                                        <div>
                                            <p
                                                ...
                                                Click an item, then a slot to
                                                move or swap it. The first eight
                                                slots are your hotbar. Resources
                                        ...
                                        {world.pinnedRecipe && (
                                            <p>
                                                ...
                                                {Object.entries(
                                                    ...
                                                    .map(
                                                        ([id, n]) =>
                                                            `${ITEMS[id as ItemId].name} ${count(world.inventory, id as ItemId)}/${n}`,
                                                    )
                                                    .join(" · ")}
                                            </p>
                                        )}
                                        ...
                                        {world.furnace && (
                                            <article>
                                                <h3>
                                                    ...
                                                    {
                                                        ITEMS[
                                                            world.furnace.output
                                                        ].name
                                                    }
                                                </h3>
                                                <p>
                                                    {world.furnace.remaining}{" "}
                                                    queued · fuel remaining{" "}
                                                    {(
                                                        world.furnace.fuelMs /
                                                        1000
                                                    ).toFixed(1)}
                                                    s
                                                </p>
                                                ...
                                                <p>
                                                    Close this menu to process
                                                    the batch. Output stays safe
                                                    when your pack is full.
                                                </p>
                                            </article>
                                        )}
                                        {shownCraftTab === "cooking" ? (
                                ...
                                {menu === "skins" && (
                                    <>
                                        <h3>Mix-and-match wardrobe</h3>
                                        <p>
                                            Each part is appearance only. Owned
                                            parts equip free; packs credit parts
                                            you already own.
                                        </p>
                                        <button
                                            onClick={() =>
                                                void transact((b) => {
                                                    b.profile.outfit = {};
                                                    return "Legacy full-body appearance restored";
                                                })
                                            }>
                                            Use full-body skin
                                        </button>
                                                            ...
                                                            <path
                                                                d={
                                                                    ...
                                                                    "head"
                                                                        ...
                                                                        : p.slot ===
                                                                            "legs"
                                                                          ? "M12 4H21V35H8V28H12Z M27 4H36V28H40V35H27Z"
                                                                          : p.slot ===
                                                                              "arms"
                                                                            ? "M4 6H15V32H4Z M33 6H44V32H33Z"
                                                                            : "M10 3H38V37H10Z"
                                                                }
                                                                fill={p.color}
                                                            />
                                        ...
                                        <h3>Weapon appearances</h3>
                                        <select
                                            aria-label="Gun family"
                                            value={gunFamily}
                                            onChange={(e) =>
                                                setGunFamily(e.target.value)
                                            }>
                                            {GUN_FAMILIES.map((id) => (
                                        ...
                                        <div className="recipe-grid">
                                            {Object.values(WEAPON_SKINS).map(
                                                (s) => {
                                                    const owned = (
                                                        profile.gunSkins ?? [
                                                            "skin_brushed_steel",
                                                    ...
                                                    return (
                                                        <article key={s.id}>
                                                            <svg
                                                                ...
                                                                />
                                                                <path
                                                                    d="M44 6H59V10H44Z M8 5H15V9H8Z"
                                                                    fill={
                                                                        s
                                                                            .palette
                                                                            .secondary
                                ...
                                {menu === "skins" && (
                                    <>
                                        <p>
                                            Cosmetics belong to your local
                                            player profile and work across
                                            worlds. Coins are spent from this
                                            world. Skins never change combat
                                            stats.
                                        </p>
                                ...
                                {menu === "objectives" && (
                                    <>
                                        <p>
                                            One-time objectives for this world.
                                            Progress comes from exploring,
                                            mining, fighting, and crafting.
                                            Claim each reward once.
                                        </p>
                                ...
                                {menu === "shop" && (
                                    <>
                                        ...
                                        <p>
                                            ...
                                            safely held here when your pack is
                                            full.
                                        </p>
                                        <button
                                            disabled={!hud.nearBase || pending}
                                            onClick={() =>
                                                void transact((_b, w) => {
                                                    let total = 0;
                                                    for (const [
                                                    ...
                                                    return total
                                                        ? `Collected ${total} held resources`
                                                        : "No held resources, or your inventory is still full.";
                                                })
                                            }>
                                ...
                                {menu === "map" && (
                                    <>
                                        <p>
                                            Only surveyed chunks are shown. ◈
                                            marks discovered caches; ● marks the
                                            outpost. Use Recall to return
                                            safely.
                                        </p>
                                        <svg
                                            viewBox="0 0 620 300"
                                            ...
                                            role="img"
                                            aria-label="Explored terrain map">
                                            {Object.entries(world.explored)
                                                .slice(-1000)
                                                .map(([key, b]) => {
                                                    const [x, y] = key
                                                            .split(",")
                                        ...
                                        <button
                                            onClick={() => {
                                                setMenu(null);
                                                engine.current?.recall();
                                            }}>
                                            Recall to outpost · 2.5 seconds
                                        </button>
                                    </>
                                )}
                                {menu === "worlds" && (
                                    <>
                                        <p>
                                            Worlds store their own items, coins,
                                            objectives, and terrain changes.
                                            Skins are shared. Saves live in this
                                            browser only; they are not
                                            cloud-synced.
                                        </p>
                                        ...
                                        <div className="world-delete-panel in-menu">
                                            <h3>Delete a world</h3>
                                            <p>
                                                You can delete every world.
                                                Deleting the last returns to
                                                world select.
                                            </p>
                                            <select
                                                value={deleteTarget}
                                        ...
                                        <p>
                                            Terrain settings are fixed once a
                                            world is created. New terrain
                                            settings require a new world.
                                        </p>
                                        <details open>
                                            <summary>
                                                ✦ AI World Creation · MIT Parley
                                                AI
                                            </summary>
                                            <div
                                                ...
                                                <label>
                                                    ...
                                                    <textarea
                                                        ...
                                                        placeholder="e.g. Higher mountain peaks, cavernous cave network, standard difficulty, abundant crystals..."
                                                        rows={3}
                                                        disabled={pending}
                                                    />
                                                </label>
                                                <button
                                                    ...
                                                    onClick={() =>
                                                        void act(async () => {
                                                            setNotice(
                                                                "Requesting world generation from MIT Parley API…",
                                                            );
                                                            const res =
                                                                await fetch(
                                                                    "/api/edit-game",
                                                            ...
                                                            if (!result.ok)
                                                                throw new Error(
                                                                    result.error ||
                                                                        "Parley API request failed.",
                                                                );
                                                            if (result.data) {
                                                                const newSettings =
                                                                    result.data as WorldSettings;
                                                                validateSettings(
                                                                    newSettings,
                                                                );
                                                                setSettings(
                                                ...
                                                <small
                                                    ...
                                                    Backend routes to MIT Parley
                                                    API using Bearer
                                                    PARLEY_API_KEY and bounded
                                                    engine validation.
                                                </small>
                                            </div>
                                        </details>
                                        ...
                                        <p>
                                            Import merges worlds and cosmetic
                                            ownership. Existing worlds are
                                            preserved as separate copies if IDs
                                            match.
                                        </p>
                                    </>
                                )}
                                ...
                                {menu === "settings" && (
                                    <>
                                        ...
                                        <p>
                                            Controls: A/D or arrows move; Space
                ...
                <div
                    ...
                    {world.inventory.slice(0, 8).map((s, i) => (
                        <button
                            key={i}
                            className={hud.selected === i ? "selected" : ""}
                            onPointerDown={(e) => e.preventDefault()}
                            onClick={() => engine.current?.select(i)}
                            title={
                                s
                ...
                <div className="reward-line" role="status">
                    <span>✦</span>
                    {hud.recall > 0
                        ? `Recall ${Math.round(hud.recall * 100)}%`
                        : notice && !menu
                          ? notice
                          : hud.message}
                </div>
            ...
            <div className="guide">
                ...
                <span>
                    Next:{" "}
                    {OBJECTIVES.find((o) => !world.claimed.includes(o.id))
                        ?.name ?? "Explore beyond the frontier"}
                </span>
            </div>
