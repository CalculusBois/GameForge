...
export function startSandbox(
    host: HTMLElement,
    store: SaveStore,
    onHud: (h: SandboxHud) => void,
    onMenu: (m: Menu) => void,
    verification?: (port: VerificationPort) => void,
): SandboxController {
    ...
    class WorldScene extends Phaser.Scene {
        ...
        busy = false;
        message =
            "Gather timber with E · Mine the nearby iron · Visit your workbench";
        noticeUntil = 0;
        sessionId = crypto.randomUUID();
        analyticsLastX = 0;
        analyticsLastY = 0;
        analyticsLastSample = 0;
        safe = { x: 0, y: 0 };
        decorDirty = false;
        constructor() {
            super("Sandbox");
        ...
        create() {
            ...
            let p = w.player;
            if (
                solid(w, Math.floor(p.x / TILE), Math.floor(p.y / TILE)) ||
                p.y >= (DEPTH - 3) * TILE
            )
                p = w.checkpoint;
            this.player = this.physics.add
                .sprite(p.x, p.y, `${store.data.profile.equipped}-idle`)
                .setDepth(10);
            this.player
                .setSize(22, 42)
                .setOffset(9, 4)
                .setMaxVelocity(MOVEMENT.speed, MOVEMENT.maxFall);
            this.safe = { x: p.x, y: p.y };
            this.analyticsLastX = p.x;
            this.analyticsLastY = p.y;
            this.weapon = this.add
                .image(p.x + 20, p.y, "gun-tool")
                .setOrigin(0.15, 0.5)
                .setDepth(11);
            ...
            this.physics.add.collider(this.shots, this.chunks.terrain, (s) => {
                this.burst(
                    (s as Phaser.Physics.Arcade.Sprite).x,
                    (s as Phaser.Physics.Arcade.Sprite).y,
                    0x82eeef,
                );
                s.destroy();
            });
            this.physics.add.collider(this.hostile, this.chunks.terrain, (s) =>
                s.destroy(),
            );
            this.physics.add.overlap(this.shots, this.foes, (s, f) => {
                const shot = s as Phaser.Physics.Arcade.Sprite,
                    foe = f as Phaser.Physics.Arcade.Sprite;
                if (!shot.active || !foe.active || foe.getData("dying")) return;
                this.hit(foe, shot.getData("damage"));
                shot.destroy();
            });
            this.physics.add.overlap(this.player, this.foes, (_p, f) => {
                const foe = f as Phaser.Physics.Arcade.Sprite;
                if (!foe.getData("dying"))
                    this.damage(
                        ENEMIES[foe.getData("kind") as Kind].damage,
                        foe.x,
                    );
            });
            this.physics.add.overlap(this.player, this.hostile, (_p, s) => {
                const shot = s as Phaser.Physics.Arcade.Sprite;
                this.damage(shot.getData("damage"), shot.x);
                shot.destroy();
            });
            this.target = this.add.graphics().setDepth(12);
            this.light = this.add.graphics().setDepth(20).setScrollFactor(0);
            this.drawBase();
            this.cameras.main.startFollow(this.player, true, 0.13, 0.13, 0, 30);
            ...
            if (import.meta.env.DEV && verification)
                verification({
                    read: () => {
                        ...
                        return {
                            ...
                            recall: this.recallStart,
                            enemies: this.foes.getChildren().map((obj) => {
                    ...
                    hold: (keys) => {
                        ...
                        for (const key of next)
                            if (!this.held.has(key)) this.fresh.add(key);
                        this.held = next;
                    },
                    aim: (x, y) => {
                        this.cursor = { x, y, known: true };
                    },
                    select: (n) => {
                        this.selected = n;
                    },
        ...
        drawBase() {
            ...
            g.fillStyle(0x1b3645);
            g.fillRect(6 * TILE, y - 104, 19 * TILE, 104);
            g.lineStyle(3, 0x83bcbb);
            g.strokeRect(6 * TILE, y - 104, 19 * TILE, 104);
            g.fillStyle(0x35515a);
            g.fillRect(6 * TILE, y - 104, 19 * TILE, 10);
            g.fillStyle(0x85eace);
            g.fillRect(8 * TILE, y - 65, 7, 60);
            for (const [x, name, color] of [
                [14, "WORKBENCH", 0xcca176],
                [18, "FORGE", 0xf0ae6d],
                [22, "TERMINAL", 0x8dd8f3],
            ] as const) {
                g.fillStyle(color);
                g.fillRect(x * TILE - 18, y - 30, 36, 30);
                this.add
                    .text(x * TILE, y - 52, name, {
            ...
            this.add
                .text(8 * TILE, y - 86, "LUMEN OUTPOST", {
                    fontFamily: "monospace",
                    fontSize: "12px",
                    color: "#94f3ce",
                })
        ...
        nearBase() {
            return (
                Math.abs(this.player.x - 15 * TILE) < 270 &&
                Math.abs(this.player.y - 22 * TILE) < 120
            );
        }
        emit() {
            ...
            onHud({
                ...
                y: this.player.y,
                recall: this.recallStart
                    ? Math.min(1, (this.clock - this.recallStart) / 2500)
                    : 0,
            });
        }
        notify(m: string) {
            this.message = m;
            this.noticeUntil = this.clock + 4500;
            this.emit();
        }
        track(
            type: AnalyticsEventType,
        ...
        async transact(action: Parameters<SaveStore["transact"]>[0]) {
            try {
                const result = await store.transact(action);
                if (typeof result === "string") this.notify(result);
                return true;
            } catch (e) {
        ...
        burst(x: number, y: number, color: number) {
            for (let i = 0; i < 5 && this.particles.size < 80; i++) {
        ...
        recall() {
            ...
            this.recallStart = this.clock || 1;
            this.track("recall_started");
            this.notify(
                "Recalling to the outpost… remain still for 2.5 seconds. Damage cancels recall.",
            );
        }
        respawn() {
            this.held.clear();
            ...
            this.cursor.known = false;
            this.health = 100;
            this.mana = 100;
            this.hurtUntil = this.clock + 2000;
            this.recallStart = 0;
            this.shots.clear(true, true);
            this.hostile.clear(true, true);
            this.foes.clear(true, true);
            this.player.setVelocity(0).setAcceleration(0);
            this.player.setPosition(
                store.world.checkpoint.x,
                store.world.checkpoint.y,
            );
            ...
            this.track("respawn");
            this.notify(
                "Returned to the outpost. Inventory, coins, and skins retained.",
            );
        }
        damage(n: number, x: number) {
            if (
                this.status !== "playing" ||
                this.clock < this.hurtUntil ||
                this.nearBase()
            )
                return;
            this.health = Math.max(
                ...
                this.health -
                    Math.ceil(
                        n *
                            (store.world.settings.difficulty === "explorer"
                                ? 0.7
                                : 1),
                    ),
            );
            ...
            this.player
                .setAccelerationX(0)
                .setVelocity(this.player.x < x ? -170 : 170, -170);
            if (this.recallStart) {
                this.recallStart = 0;
                this.track("recall_interrupted");
            ...
            if (this.shake) this.cameras.main.shake(100, 0.003);
            if (!this.health) {
                this.status = "dead";
                this.track("death");
                this.held.clear();
                this.physics.pause();
                this.notify(
                    "Signal lost. Respawn at the beacon; all possessions are retained.",
                );
            }
            this.emit();
        }
        hit(f: Phaser.Physics.Arcade.Sprite, damage: number) {
            if (f.getData("dying")) return;
            const kind = f.getData("kind") as Kind;
            const vulnerable =
                kind === "caster" && f.getData("state") === "recover";
            ...
            this.track("attack_hit", {
                kind,
                damage: damage * (vulnerable ? 1.5 : 1),
                killed: hp <= 0,
            });
            if (hp <= 0) {
                ...
                void this.transact((_b, w) => {
                    ...
                    recordAnalytics(w, "coins_changed", {
                        amount: ENEMIES[kind].reward,
                        reason: "enemy",
                        kind,
                    });
                    const resource: ItemId =
                        kind === "caster"
                            ? "crystal"
                            : kind === "hopper"
                              ? "herb"
                    ...
                    if (!stored) {
                        w.pendingLoot ??= {};
                        w.pendingLoot[resource] =
                            (w.pendingLoot[resource] ?? 0) + 1;
                    }
        ...
        attack() {
            ...
            if (item === "tonic") {
                ...
                if (!this.busy) {
                    this.busy = true;
                    void this.transact((_b, w) => {
                        if (!remove(w.inventory, "tonic", 1))
                            throw new Error("No field tonic.");
                        return "Restored 40 health";
                    }).then((ok) => {
                        if (disposed) return;
                        if (ok) this.health = Math.min(100, this.health + 40);
                        this.track("item_spent", {
                            item: "tonic",
                            quantity: 1,
                        });
                        this.busy = false;
            ...
            if (this.clock - this.lastAttack < stats.cooldown) return;
            if (this.mana < stats.mana) {
                this.lastAttack = this.clock;
                this.notify("Low mana. Let your energy recharge.");
                return;
            }
            if (this.shots.countActive() >= 30) return;
            this.lastAttack = this.clock;
            this.track("attack_fired", { item });
            this.mana -= stats.mana;
            if (stats.mana) this.lastMagic = this.clock;
            if (item === "sword") {
                const arc = this.add.graphics().setDepth(12);
                arc.lineStyle(5, 0xb4fff0, 0.9);
                arc.beginPath();
                arc.arc(
                    this.player.x,
                    this.player.y,
                ...
                for (const obj of this.foes.getChildren()) {
                    const f = obj as Phaser.Physics.Arcade.Sprite;
                    if (
                        Math.abs(f.x - this.player.x) < 80 &&
                        Math.abs(f.y - this.player.y) < 60 &&
                        (f.x - this.player.x) * this.facing >= -12 &&
                        clearLine(
                            store.world,
                            this.player.x,
                            this.player.y,
        ...
        targetTile() {
            return {
                x: Math.floor(
                    (this.cursor.known
                        ? this.cursor.x
                        : this.player.x + this.facing * 32) / TILE,
                ),
                y: Math.floor(
                    (this.cursor.known ? this.cursor.y : this.player.y + 32) /
                        TILE,
                ),
            };
        }
        inReach(x: number, y: number) {
            return (
                Math.hypot(
                    (x + 0.5) * TILE - this.player.x,
                    (y + 0.5) * TILE - this.player.y,
                ) <=
                    TILE * 5 &&
                clearLine(
                    store.world,
        ...
        mine(delta: number) {
            ...
            const { x, y } = this.targetTile(),
                m = readTile(store.world, x, y),
                key = `${x},${y}`;
            if (!this.inReach(x, y) || !m || m === 6 || protectedTile(x, y)) {
                this.mining.progress = 0;
                return;
            }
            if (this.mining.key !== key) this.mining = { key, progress: 0 };
            this.mining.progress += delta * (item === "drill" ? 2 : 1);
            this.target.fillStyle(0xf1e0b0, 0.8);
            this.target.fillRect(
                x * TILE,
                y * TILE - 5,
                TILE * Math.min(1, this.mining.progress / MATERIALS[m].time),
                3,
            );
            if (this.mining.progress >= MATERIALS[m].time && !this.busy) {
                ...
                this.mining.progress = 0;
                void this.transact((_b, w) => {
                    if (readTile(w, x, y) !== m) return "";
                    const drop = MATERIALS[m].drop;
                    if (drop && !add(w.inventory, drop, 1))
                        throw new Error(
                            "Inventory full. The block was not mined.",
                        );
                    editTile(w, x, y, 0);
                    if (drop === "stone") w.progress.stone++;
        ...
        place() {
            ...
            if (!item || BUILDING[item] === undefined) {
                this.notify(
                    "Select soil, stone, outpost blocks, or torches to place.",
                );
                return;
            }
            const material = BUILDING[item]!,
                { x, y } = this.targetTile();
            ...
            const occupied =
                ...
                this.foes
                    ...
                    .some((f) =>
                        Phaser.Geom.Intersects.RectangleToRectangle(
                            rect,
                            (f as Phaser.Physics.Arcade.Sprite).getBounds(),
                        ),
                    );
            if (
                this.busy ||
                !this.inReach(x, y) ||
                readTile(store.world, x, y) !== 0 ||
                occupied ||
                protectedTile(x, y) ||
                ![
                    [1, 0],
                    ...
                    [0, 1],
                    [0, -1],
                ].some(([dx, dy]) => solid(store.world, x + dx, y + dy))
            ) {
                this.notify(
                    "Place beside an existing block, within reach, away from actors and the outpost.",
                );
                return;
            }
            this.busy = true;
            void this.transact((_b, w) => {
                if (readTile(w, x, y) !== 0 || !remove(w.inventory, item, 1))
                    throw new Error("Placement unavailable.");
                editTile(w, x, y, material);
        ...
        interact() {
            const w = store.world,
                cx = Math.floor(this.player.x / (TILE * CHUNK));
            for (const dx of [-1, 0, 1]) {
                for (const l of landmarks(w.settings, cx + dx)) {
                    if (
                        Math.hypot(
                            l.x * TILE - this.player.x,
                            l.y * TILE - this.player.y,
                        ) < 80 &&
                        clearLine(
                            w,
                            this.player.x,
                        ...
                        if (!this.busy) {
                            ...
                            void this.transact((_b, world) =>
                                ...
                                if (disposed) return;
                                if (ok) this.chunks.refreshDecor(l.x, l.y);
                                this.busy = false;
                            });
                        }
                        return;
                ...
                for (const p of harvestables(w.settings, cx + dx)) {
                    if (
                        !w.harvested.includes(p.id) &&
                        Math.hypot(
                            p.x * TILE - this.player.x,
                            p.y * TILE - 20 - this.player.y,
                        ) < 75 &&
                        clearLine(
                            w,
                            this.player.x,
                            ...
                            const id: ItemId =
                                    p.kind === "tree"
                                        ...
                                        : p.kind === "herb"
                                          ? "herb"
                                          : "scrap",
                                quantity = p.kind === "tree" ? 8 : 3;
                            if (!add(world.inventory, id, quantity))
                                throw new Error(
                                    "Inventory full. Gather this later.",
                                );
            ...
            if (this.nearBase()) {
                this.pause();
                onMenu(
                    Math.abs(this.player.x - 22 * TILE) < 55
                        ? "shop"
                        : "crafting",
                );
                return;
            ...
            this.notify(
                "Press E beside a tree, herb, salvage pile, chest, or outpost station.",
            );
        }
        spawn() {
            if (this.foes.countActive() >= 12) return;
            const w = store.world,
                cam = this.cameras.main.worldView;
            for (const key of this.chunks.active.keys()) {
                const [cx, cy] = key.split(",").map(Number),
                    id = `enemy:${key}`;
                if (
                    w.defeated.includes(id) ||
                    this.foes
                        .getChildren()
                        .some(
                            (f) =>
                                (f as Phaser.Physics.Arcade.Sprite).getData(
                                    "id",
                                ) === id,
                        )
                )
                ...
                const tx =
                    cx * CHUNK +
                    6 +
                    Math.floor(hash(w.settings.seed, cx, cy, "enemy") * 20);
                ...
                if (Math.floor(ty / CHUNK) !== cy) continue;
                for (let n = 0; n < 7 && !solid(w, tx, ty + 1); n++) ty++;
                const x = (tx + 0.5) * TILE,
                    y = (ty + 1) * TILE - 23;
                if (
                    Math.abs(x - 12 * TILE) < 580 ||
                    Phaser.Geom.Rectangle.Contains(
                        Phaser.Geom.Rectangle.Clone(cam).setSize(
                            cam.width + 100,
                            cam.height + 100,
                        ),
                        x,
                    ...
                    Math.hypot(x - this.player.x, y - this.player.y) > 1600 ||
                    solid(w, tx, ty) ||
                    solid(w, tx, ty - 1) ||
                    !solid(w, tx, ty + 1)
                )
                ...
                const b = biome(w.settings, tx, ty),
                    roster: Kind[] =
                        b === "Crystal depths"
                            ? ["caster", "sentinel", "drone"]
                            : b === "Rust wastes"
                              ? ["crawler", "gunner", "drone"]
                              : ["crawler", "hopper"];
                const kind =
                    roster[
                        Math.floor(
                            hash(w.settings.seed, tx, ty, "roster") *
                                roster.length,
                        )
                    ];
                this.createEnemy(kind, x, y, id);
        ...
        createEnemy(kind: Kind, x: number, y: number, id: string) {
            ...
            f.setData({
                ...
                nextThink:
                    this.clock +
                    hash(store.world.settings.seed, x, y, "ai-phase") * 100,
            });
            if (kind === "drone")
        ...
        ai() {
            for (const obj of this.foes.getChildren()) {
                ...
                if (
                    dist > 1800 ||
                    !this.chunks.has(
                        Math.floor(f.x / (CHUNK * TILE)),
                        Math.floor(f.y / (CHUNK * TILE)),
                    )
                ) {
                    f.destroy();
                    continue;
                ...
                let direction =
                    Math.sign(
                        (sight && pursuit
                            ? this.player.x
                            : this.clock < f.getData("memory") && pursuit
                              ? f.getData("lastSeen")
                              : home) - f.x,
                    ) || 1;
                ...
                if (state === "windup") {
                    f.setVelocityX(0);
                    f.setTint(0xffbc77);
                    f.setScale(1.08, 0.93);
                    if (this.clock < until) continue;
                    if (kind === "hopper") {
                        const landingX = Math.floor(
                            (f.x + direction * 85) / TILE,
                        );
                        const landingY = Math.floor(body.bottom / TILE);
                        const safeLanding = [0, 1, 2, 3].some(
                            (dy) =>
                                solid(store.world, landingX, landingY + dy) &&
                                !solid(
                                    store.world,
                                    landingX,
                                    landingY + dy - 1,
                        ...
                        if (sight && this.hostile.countActive() < 24) {
                            const shot = this.hostile.create(
                                f.x,
                                f.y,
                                kind === "caster" ? "magic" : "hostile",
                            ) as Phaser.Physics.Arcade.Sprite;
                            shot.setTint(0xff847a);
                            const v = new Phaser.Math.Vector2(dx, dy)
                                .normalize()
                                .scale(kind === "caster" ? 145 : 220);
                            shot.setVelocity(v.x, v.y);
                            shot.setData({
                                damage: p.damage,
                                expires: this.clock + 2400,
                            });
                        }
                ...
                if (state === "burst") {
                    ...
                    if (this.clock >= until) {
                        ...
                        f.setData({
                            remaining,
                            state: remaining > 0 ? "burst" : "recover",
                            until: this.clock + (remaining > 0 ? 180 : 1300),
                        });
                    }
                    continue;
                }
                if (state === "charge") {
                    if (
                        ...
                        solid(
                            store.world,
                            Math.floor(
                                (f.x + Math.sign(body.velocity.x) * 32) / TILE,
                            ),
                            Math.floor((body.bottom + 8) / TILE),
                        )
                    )
                        continue;
                    state = "recover";
                ...
                if (state === "recover" && this.clock < until) {
                    if (kind === "drone") f.setVelocityX(-direction * 60);
                    else if (kind !== "hopper") f.setVelocityX(0);
                    continue;
                }
                if (
                    sight &&
                    ...
                    if (
                        kind === "hopper" ||
                        (kind === "sentinel" && dist < 120) ||
                        (kind === "caster" && dist < 350) ||
                        (kind === "gunner" && dist < 370) ||
                        (kind === "drone" && dist < 260)
                    ) {
                        f.setData({
                            state: "windup",
                ...
                state =
                    sight && pursuit
                        ...
                        : this.clock < f.getData("memory") && pursuit
                          ? "investigate"
                          : Math.abs(f.x - home) > 100
                            ? "return"
                            : "patrol";
                f.setData("state", state);
                if (
                    sight &&
                    (kind === "caster" || kind === "gunner") &&
                    dist < 160
                )
                ...
                if (kind === "drone") {
                    const targetY =
                        sight && pursuit
                            ? this.player.y - 45
                            : f.getData("homeY") - 40;
                    let vx = direction * p.speed,
                        vy = Phaser.Math.Clamp((targetY - f.y) * 1.5, -65, 65);
                    if (
                        !clearLine(
                            store.world,
                            f.x,
                    ...
                    const tx = Math.floor((f.x + direction * 24) / TILE),
                        ty = Math.floor((body.bottom + 8) / TILE),
                        floor = solid(store.world, tx, ty);
                    if (body.blocked.left || body.blocked.right) {
                        if (
                            sight &&
                            (body.blocked.down || body.touching.down) &&
                            !solid(store.world, tx, ty - 2) &&
                            solid(store.world, tx, ty - 1)
                        )
                            f.setVelocityY(-330);
        ...
        drawEnvironment() {
            ...
            const tx = Math.floor(this.player.x / TILE),
                ...
                b = biome(store.world.settings, tx, ty),
            ...
            this.stars.fillStyle(
                b === "Rust wastes" ? 0x765547 : 0x2d5261,
                0.5,
            );
            for (let i = -1; i < 9; i++) {
                const x = i * 190 - ((cam.scrollX * 0.15) % 190);
                this.stars.fillTriangle(
                    x,
                    430,
                    x + 115,
                    150 + (i % 3) * 55,
            ...
            this.stars.fillStyle(0xc6e9d5, 0.6);
            for (let i = 0; i < 16; i++)
                this.stars.fillCircle(
                    (i * 173 - cam.scrollX * 0.06) % 1000,
                    50 + ((i * 67) % 300),
                    1,
                );
            this.light.clear();
                ...
                for (let x = tx - 23; x <= tx + 23; x++)
                    for (let y = ty - 13; y <= ty + 13; y++) {
                        ...
                        if (
                            material === 8 ||
                            (material === 4 &&
                                hash(store.world.settings.seed, x, y, "glow") >
                                    0.9)
                        )
                            sources.push({
                                x: (x + 0.5) * TILE - cam.scrollX,
                                y: (y + 0.5) * TILE - cam.scrollY,
                                r: material === 8 ? 145 : 65,
                            });
                    }
        ...
        update(_t: number, delta: number) {
            if (!this.player || disposed) return;
            this.chunks.ensure(this.player.x, this.player.y);
            this.chunks.step();
            if (this.decorDirty) {
            ...
            const m = MOVEMENT,
                b = this.player.body as Phaser.Physics.Arcade.Body,
            ...
            if (this.fresh.has("Space")) this.buffer = this.clock;
            if (
                this.clock - this.buffer <= m.bufferMs &&
                this.clock - this.lastGround <= m.coyoteMs &&
                !this.jumped
            ) {
            ...
            if (this.clock >= this.knockUntil) {
                this.player
                    .setAccelerationX(direction * m.acceleration)
                    .setDragX(direction ? 0 : m.deceleration);
            }
            if (direction) {
            ...
            this.player.setAlpha(
                hurt && Math.floor(this.clock / 80) % 2 ? 0.45 : 1,
            );
            this.player.setTexture(
                `${store.data.profile.equipped}-${hurt ? "hurt" : !grounded ? (b.velocity.y < 0 ? "air" : "fall") : direction ? `run${Math.floor(this.clock / 100) % 2}` : "idle"}`,
            );
            const item = store.world.inventory[this.selected]?.id;
            this.weapon
                .setTexture(
            ...
            if (
                ...
                this.target.lineStyle(
                    2,
                    this.inReach(x, y) ? 0x8df6d1 : 0xef998c,
                    0.8,
                );
            ...
            if (key !== this.lastExplore) {
                ...
                void this.transact((_b, w) => {
                    ...
                    w.explored[key] = name;
                    if (name !== "Verdant frontier") w.progress.biome = 1;
                });
            }
            if (
                this.player.y > (DEPTH - 2) * TILE ||
                Math.abs(this.player.x) > WORLD_LIMIT * TILE - 80
            ) {
                this.health = 0;
                this.status = "dead";
                this.physics.pause();
            ...
            if (this.clock - this.lastHud > 180) {
                ...
                if (this.clock - this.analyticsLastSample >= 5000) {
                    const distance = Math.hypot(
                        this.player.x - this.analyticsLastX,
                        this.player.y - this.analyticsLastY,
                    );
                    this.analyticsLastX = this.player.x;
                    this.analyticsLastY = this.player.y;
                    this.analyticsLastSample = this.clock;
                    this.track("position_sample", {
                        distance,
                        depth: Math.max(
                            0,
                            Math.floor(this.player.y / TILE) -
                        ...
                        distanceFromBase: Math.hypot(
                            this.player.x - store.world.checkpoint.x,
                            this.player.y - store.world.checkpoint.y,
                        ),
                    });
                }
                store.schedulePosition(
                    this.player.x,
                    this.player.y,
                    this.selected,
                );
    ...
    const game = new Phaser.Game({
        ...
        physics: {
            default: "arcade",
            arcade: { gravity: { x: 0, y: MOVEMENT.gravity }, fps: 120 },
        },
    ...
    const keydown = (e: KeyboardEvent) => {
        if (document.activeElement !== host || !scene) return;
        if (
            [
                "Space",
                "ArrowLeft",
        ...
        if (e.code === "Escape") {
            if (scene.status === "paused") scene.resume();
            else scene.pause();
            return;
        }
        ...
        if (["KeyA", "KeyD", "ArrowLeft", "ArrowRight"].includes(e.code))
            scene.cursor.known = false;
        scene.held.add(e.code);
        scene.fresh.add(e.code);
    };
    const keyup = (e: KeyboardEvent) => scene?.held.delete(e.code);
    const move = (e: PointerEvent) => {
        if (!scene?.player) return;
        const r = host.getBoundingClientRect();
        const p = scene.cameras.main.getWorldPoint(
            ((e.clientX - r.left) * 960) / r.width,
            ((e.clientY - r.top) * 540) / r.height,
        );
        scene.cursor = { x: p.x, y: p.y, known: true };
    };
    const down = (e: PointerEvent) => {
        host.focus();
        if (!scene || scene.status !== "playing") return;
        move(e);
        if (e.button === 0) scene.held.add("Mouse");
        if (e.button === 2) scene.fresh.add("RightMouse");
    };
    const up = () => scene?.held.delete("Mouse");
    const blur = () => {
        if (!verification) scene?.pause();
    };
    const hidden = () => {
