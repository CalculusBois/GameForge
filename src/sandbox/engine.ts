...
export function startSandbox(host: HTMLElement, store: SaveStore, onHud: (h: SandboxHud) => void, onMenu: (m: Menu) => void, verification?: (port:VerificationPort)=>void): SandboxController {
    ...
    class WorldScene extends Phaser.Scene {
        ...
        gathering = { id: '', progress: 0, kind: 'herb' as 'tree' | 'herb' | 'scrap' };
        lastMineSound = 0;
        lastWalkSound = 0;
        fallPeak = 0;
        fallStartY = 0;
        ...
        pause() { if (this.status === 'dead')
            return; this.status = 'paused'; this.physics.pause(); this.held.clear(); this.fresh.clear(); this.tweens.pauseAll(); this.recallStart = 0; gameAudio.stopBgm(); this.emit(); void this.save().catch(() => { }); }
        resume() { if (this.status === 'dead')
            return; this.status = 'playing'; this.held.clear(); this.fresh.clear(); this.tweens.resumeAll(); this.physics.resume(); gameAudio.unlock(); gameAudio.startBgm(); this.emit(); }
        ...
        hit(f: Phaser.Physics.Arcade.Sprite, damage: number) { if (f.getData('dying'))
            ...
            f.setData('dying', true); if (kind === 'bomber') this.explode(f.x, f.y);
            gameAudio.death();
            f.setVelocity(0);
        ...
        attack() {
            ...
            if (item === 'tonic') {
                ...
                if (!this.busy) {
                    ...
                    void this.transact((_b, w) => { if (!remove(w.inventory, 'tonic', 1))
                        throw new Error('No field tonic.'); return 'Restored 20 health'; }).then(ok => { if (disposed)
                        return; if (ok)
                        this.health = Math.min(100, this.health + 20); this.busy = false; });
                }
                return;
            }
            if (!(item in WEAPONS))
            ...
            if (item !== 'sword' && this.shots.countActive() >= 30)
                return;
            const aim = this.cursor.known ? new Phaser.Math.Vector2(this.cursor.x - this.player.x, this.cursor.y - this.player.y) : new Phaser.Math.Vector2(this.facing, 0);
            if (aim.lengthSq() < 1) aim.set(this.facing, 0);
            else aim.normalize();
            if (item === 'sword') {
                this.lastAttack = this.clock;
                gameAudio.weapon('melee');
                const swing = Math.atan2(aim.y, aim.x);
                const arc = this.add.graphics().setDepth(12);
                arc.lineStyle(5, 0xb4fff0, .9);
                arc.beginPath();
                arc.arc(this.player.x, this.player.y, 62, swing - 0.95, swing + 0.95);
                arc.strokePath();
                ...
                for (const obj of this.foes.getChildren()) {
                    ...
                    if (dist > 88 || dist < 1) continue;
                    const aligned = toFoe.normalize().dot(aim);
                    if (aligned > 0.15 && clearLine(store.world, this.player.x, this.player.y, f.x, f.y))
            ...
            if (!clearLine(store.world, this.player.x, this.player.y, x, y)) {
                spawnX = this.player.x + aim.x * 12;
                spawnY = this.player.y + aim.y * 12;
                if (!clearLine(store.world, this.player.x, this.player.y, spawnX, spawnY))
                    return;
            }
            this.lastAttack = this.clock;
            ...
            gameAudio.weapon(item === 'staff' ? 'magic' : 'ranged');
            const shot = this.shots.create(spawnX, spawnY, item === 'staff' ? 'magic' : 'shot') as Phaser.Physics.Arcade.Sprite;
            ...
            this.burst(spawnX, spawnY, item === 'staff' ? 0xc2a3ff : 0x9dffdf);
        }
        ...
        mine(delta: number) {
            ...
            if (this.clock - this.lastMineSound > 150) {
                this.lastMineSound = this.clock;
                gameAudio.mine(m);
            }
            ...
            if (this.mining.progress >= MATERIALS[m].time && !this.busy) {
                ...
                void this.transact((_b, w) => { if (readTile(w, x, y) !== m)
                    ...
                    throw new Error('Inventory full. The block was not mined.'); editTile(w, x, y, 0); clearUnsupportedHarvest(w, [[x, y]]); if (drop === 'stone')
        ...
        interact() {
            ...
            if (!this.findHarvest())
                this.notify('Hold E beside a tree, herb, or salvage pile · Press E at a chest or outpost.');
        }
        findHarvest() {
            const w = store.world, cx = Math.floor(this.player.x / (TILE * CHUNK));
            for (const dx of [-1, 0, 1]) {
                for (const p of harvestables(w.settings, cx + dx)) {
                    if (w.harvested.includes(p.id) || !solid(w, p.x, p.y)) continue;
                    if (Math.hypot(p.x * TILE - this.player.x, p.y * TILE - 20 - this.player.y) < 75 && clearLine(w, this.player.x, this.player.y, p.x * TILE, p.y * TILE - 15))
                        return p;
                }
            }
            return null;
        ...
        gather(delta: number) {
            const p = this.findHarvest();
            if (!p) {
                this.gathering = { id: '', progress: 0, kind: 'herb' };
                return;
            }
            if (this.gathering.id !== p.id)
                this.gathering = { id: p.id, progress: 0, kind: p.kind };
            const need = HARVEST_MS[p.kind];
            this.gathering.progress += delta;
            this.target.fillStyle(0xb7e8a8, .85);
            this.target.fillRect(p.x * TILE, p.y * TILE - 28, TILE * Math.min(1, this.gathering.progress / need), 4);
            if (this.gathering.progress < need || this.busy) return;
            this.busy = true;
            this.gathering.progress = 0;
            void this.transact((_b, world) => {
                if (world.harvested.includes(p.id) || !solid(world, p.x, p.y)) return '';
                const id: ItemId = p.kind === 'tree' ? 'wood' : p.kind === 'herb' ? 'herb' : 'scrap';
                const quantity = p.kind === 'tree' ? 8 : 3;
                if (!add(world.inventory, id, quantity))
                    throw new Error('Inventory full. Gather this later.');
                world.harvested.push(p.id);
                return `+${quantity} ${ITEMS[id].name}`;
            }).then(ok => {
                if (disposed) return;
                if (ok) this.chunks.refreshDecor(p.x, p.y - 1);
                this.busy = false;
            });
        }
        spawn() {
            ...
            for (const key of this.chunks.active.keys()) {
                ...
                for (let slot = 0; slot < slots; slot++) {
                    ...
                    if (w.defeated.includes(id) || this.foes.getChildren().some(f => (f as Phaser.Physics.Arcade.Sprite).getData('id') === id))
                        continue;
                    const tx = cx * CHUNK + 4 + Math.floor(hash(w.settings.seed, cx, cy, `enemy-${slot}`) * 24);
        ...
        drawEnvironment() {
            ...
            this.sky.clear();
            const color = b === 'Rust wastes' ? 0x3a2e28 : b === 'Crystal depths' ? 0x14141f : 0x1a2a22;
            this.sky.fillStyle(color);
            this.sky.fillRect(0, 0, 960, 540);
            this.stars.clear();
            this.stars.fillStyle(b === 'Rust wastes' ? 0x5a4538 : 0x243830, .55);
            for (let i = -1; i < 9; i++) {
                const x = i * 190 - ((cam.scrollX * .15) % 190);
                this.stars.fillTriangle(x, 430, x + 115, 150 + (i % 3) * 55, x + 260, 430);
            }
            this.stars.fillStyle(0xc6e9d5, .45);
            for (let i = 0; i < 16; i++)
                ...
                for (let x = 0; x < 960; x += 32)
                    for (let y = 0; y < 540; y += 32) {
                        ...
                        for (const light of sources)
                            alpha = Math.min(alpha, Math.max(0, Math.hypot(x + 16 - light.x, y + 16 - light.y) / light.r - .35) * .62);
                        if (alpha > .02) {
                            this.light.fillStyle(0x0a0a0c, alpha);
        ...
        update(_t: number, delta: number) {
            ...
            if (direction) {
                ...
                if (grounded && this.clock - this.lastWalkSound > 280) {
                    this.lastWalkSound = this.clock;
                    gameAudio.walk();
                }
                if (this.recallStart) {
                    this.recallStart = 0;
    ...
    const blur = () => { if (!verification && (document.hidden || !document.hasFocus())) scene?.pause(); };
