...
import { DEFAULT_WORLD, CHUNK, DEPTH, ITEMS, newWorld, initialBundle, add, count, craft, unlock, claim, chest, type ItemId } from './model';
import { generateChunk, baseTile, readTile, editTile, chunkOf, localOf, surface, landmarks, harvestables } from './terrain';
...
describe('inventory and durable progression rules', () => {
    it('consumes recipe ingredients exactly once', () => { const w = newWorld('w', DEFAULT_WORLD); add(w.inventory, 'iron', 4); craft(w, 'bar', true); expect(count(w.inventory, 'iron')).toBe(2); expect(count(w.inventory, 'bar')).toBe(1); expect(() => craft(w, 'staff', false)).toThrow(); });
    it('keeps all ingredients if crafting output does not fit', () => { const w = newWorld('w', DEFAULT_WORLD); w.inventory = Array.from({ length: 32 }, () => ({ id: 'stone' as ItemId, count: 99 })); w.inventory[0] = { id: 'iron', count: 99 }; const before = structuredClone(w.inventory); expect(() => craft(w, 'bar', true)).toThrow(/full/); expect(w.inventory).toEqual(before); });
    ...
    it('prevents repeated chest and objective rewards', () => { const w = newWorld('w', DEFAULT_WORLD); chest(w, 'landing-cache'); expect(() => chest(w, 'landing-cache')).toThrow(); expect(w.coins).toBe(35); claim(w, 'chest'); expect(() => claim(w, 'chest')).toThrow(); expect(w.coins).toBe(80); });
    ...
    it('rejects incompatible and malformed saves without modifying them', () => { const b = initialBundle(); b.version = 99; const before = JSON.stringify(b); expect(() => validateBundle(b)).toThrow(/unsupported/); expect(JSON.stringify(b)).toBe(before); });
});
